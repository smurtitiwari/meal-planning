...
import { X, Plus, Check, Trash2, RefreshCw, Repeat2 } from 'lucide-react'
...
/* ─── Accessible color palette (same as Home) ─── */
const lightColors = {
  textPrimary: '#111111',
  ...
  card: '#F6F6F6',
  border: '#F4F4F4',
  borderActive: 'rgba(60, 21, 26, 0.24)',
  secondarySurface: '#F6F6F6',
  elevatedSurface: '#FBFBFB',
  accentSoft: '#FBF5F6',
  accentText: '#111111',
...
const GROCERY_CATEGORY_EMOJI: Record<string, string> = {
  Vegetables: '🥬',
  'Dairy & Eggs': '🥛',
  'Pulses & Grains': '🌾',
  'Spices & Staples': '🧂',
...
function getCategoryForItem(name: string) {
...
export default function Planner() {
  ...
  const selectedSecondaryChipStyles = {
    ...
    boxShadow: 'none',
    border: `1px solid ${colors.pageSurface === '#121212' ? 'rgba(240, 199, 207, 0.24)' : 'rgba(60, 21, 26, 0.14)'}`,
  }
  ...
  /* ─── Grocery: Today vs Week filtering ─── */
  ...
  const todayIngredients = useMemo(() => {
    ...
    meals.forEach((meal) => {
      meal?.ingredients.forEach((ing) => ings.add(ing.toLowerCase()))
    })
    return ings
  }, [todayPlan])

  const filteredGrocery = useMemo(() => {
    if (groceryScope === 'week') return groceryList
    return groceryList.filter((item) => item.source === 'manual' || todayIngredients.has(item.name.toLowerCase()))
  ...
  const handleAddGrocery = () => {
    if (newGroceryName.trim()) {
      addGroceryItem({
        ...
        unit: newGroceryUnit.trim() || undefined,
        checked: false,
        source: 'manual',
      })
      setNewGroceryName('')
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: colors.pageSurface }}>
      ...
      <div className="px-5 mt-4 mb-3">
        <SegmentedControl
          options={[
            ...
            { value: 'grocery', label: `Grocery (${unchecked.length})` },
          ]}
          value={activeTab}
          ...
          {weeklyPlan.map((day, i) => {
            ...
            return (
              <button key={day.date} onClick={() => setSelectedDate(day.date)}
                ...
                style={{
                  background: isSel ? colors.accentSoft : (preferences.darkMode ? '#1B1B1B' : '#F6F6F6'),
                  color: isSel ? colors.accentText : colors.textPrimary,
                  border: `1px solid ${isSel ? (preferences.darkMode ? 'rgba(240, 199, 207, 0.34)' : 'rgba(60, 21, 26, 0.22)') : (preferences.darkMode ? 'rgba(255,255,255,0.1)' : '#F6F6F6')}`,
                  boxShadow: 'none',
                }}>
                <span style={{ fontSize: '10px', fontWeight: 600, opacity: isSel ? 0.72 : 0.82, textTransform: 'uppercase' }}>
                  {DAYS[i]}
                </span>
                ...
                <div
                  style={{
                    ...
                    height: 4,
                    borderRadius: 2,
                  background: isSel ? colors.accentPurple : (isToday ? colors.accentPurple : 'transparent'),
                    marginTop: 2,
      ...
      {activeTab === 'meals' ? (
        /* ===== MEALS TAB — cards consistent with Home ===== */
          ...
          {selectedPlan && mealTypes.map((type) => {
            ...
            return (
              <PlannerMealCard
                ...
                type={type}
                meal={meal}
                isDone={selectedPlan.done?.[type]}
                isSkipped={selectedPlan.skipped?.[type]}
                onNavigate={() => navigate(`/meal/${selectedDate}/${type}`)}
                onRotate={() => rotateMeal(selectedDate, type)}
                onSwap={() => {
                  setPendingMeal(selectedPlan[type])
                  setSwapModal({ date: selectedDate, type })
                }}
              />
            )
          })}
        ...
        <div className="px-5">
          ...
          <div className="flex gap-2 mb-4">
            {(['today', 'week'] as const).map((scope) => (
              <button key={scope} onClick={() => setGroceryScope(scope)}
                ...
                style={{
                  ...(groceryScope === scope ? selectedSecondaryChipStyles : inactiveSecondaryChipStyles),
          ...
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <button onClick={() => setShowAddGrocery(true)}
              className="flex items-center gap-1.5 px-3 py-2 rounded-full border cursor-pointer"
              style={{ background: colors.card, borderColor: colors.border, color: colors.textPrimary, fontSize: '12px', fontWeight: 600 }}>
              <Plus size={14} />
              Add item
            </button>
          </div>
          ...
          {unchecked.length > 0 && (
            <div className="mb-5">
              <p style={{ fontSize: '11px', fontWeight: 700, color: colors.tertiaryAction, textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 10px 0' }}>
                To buy ({unchecked.length})
              </p>
              <div className="space-y-4">
                {groupedUnchecked.map(([category, items]) => (
                  <div key={category}>
                    <p style={{ fontSize: '11px', fontWeight: 700, color: colors.textSecondary, margin: '0 0 8px 2px' }}>
                        ...
                        <div key={item.id} className="flex items-center justify-between px-4 py-3 rounded-xl"
                          ...
                          <div className="flex items-center gap-3">
                            <button onClick={() => toggleGroceryItem(item.id)}
                              ...
                              style={{ border: `2px solid ${colors.border}`, background: 'transparent' }} />
                            <span style={{ fontSize: '14px', fontWeight: 500, color: colors.textPrimary }}>{formatGroceryItem(item)}</span>
                          </div>
                          {item.source === 'manual' ? (
          ...
          {checked.length > 0 && (
            <div className="mb-4">
              <p style={{ fontSize: '11px', fontWeight: 700, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 10px 0' }}>
                Done ({checked.length})
              </p>
              <div className="space-y-4">
                {groupedChecked.map(([category, items]) => (
                  <div key={category}>
                    ...
                    <div className="space-y-1.5">
                      {items.map((item) => (
                        <div key={item.id} className="flex items-center justify-between px-4 py-3 rounded-xl"
                          style={{ background: preferences.darkMode ? colors.elevatedSurface : '#F6F6F6' }}>
                          <div className="flex items-center gap-3">
                            <button onClick={() => toggleGroceryItem(item.id)}
                            ...
                            <span style={{ fontSize: '14px', fontWeight: 500, color: colors.textSecondary, textDecoration: 'line-through' }}>{formatGroceryItem(item)}</span>
                          </div>
                          {item.source === 'manual' ? (
                            <button onClick={() => removeGroceryItem(item.id)}
                              className="bg-transparent border-none cursor-pointer p-1" style={{ color: '#D5D1DC' }}>
      ...
      {swapModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-5" onClick={() => { setSwapModal(null); setPendingMeal(null) }}>
          <div className="w-full max-w-md mx-auto rounded-3xl max-h-[80vh] overflow-hidden"
            ...
            <div className="px-5 pt-5 pb-3 flex items-center justify-between sticky top-0 z-10 border-b" style={{ borderColor: colors.border, background: colors.card }}>
              <h3 style={{ fontFamily: serifFont, fontSize: '20px', fontWeight: 400, margin: 0, color: colors.textPrimary }}>
                Replace {mealLabel[swapModal.type].toLowerCase()}
              </h3>
              <button onClick={() => { setSwapModal(null); setPendingMeal(null) }}
                className="w-8 h-8 rounded-full flex items-center justify-center border-none cursor-pointer"
                style={{ background: preferences.darkMode ? colors.elevatedSurface : '#EFE9E0', color: colors.textSecondary }}>
                <X size={16} />
              </button>
            </div>
              ...
              {getMealPool(recipes, swapModal.type).map((meal) => {
                ...
                return (
                  <button key={meal.id} onClick={() => handleSwap(meal)}
                    className="w-full flex items-center gap-3 p-3 rounded-2xl cursor-pointer border transition-smooth mb-2"
                    style={{
                      background: colors.card,
                      borderColor: meal.id === current?.id ? colors.accentPurpleLight : colors.border,
                      borderWidth: meal.id === current?.id ? 2 : 1,
                      boxShadow: meal.id === current?.id ? '0 4px 14px rgba(60,21,26,0.12)' : 'none',
                    }}>
                    <div style={{ width: 56, height: 56, borderRadius: 12, overflow: 'hidden', flexShrink: 0 }}>
                      {meal.image ? (
                        ...
                        <div style={{
                          width: '100%',
                          height: '100%',
                          background: preferences.darkMode ? colors.elevatedSurface : '#FBF5F6',
                          display: 'flex',
                          alignItems: 'center',
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <div className="flex items-center gap-2">
                        {meal.cookTime && (
                          <span style={{ fontSize: '11px', color: colors.textSecondary }}>{meal.cookTime}</span>
                        )}
                        {meal.calories && (
                          <span style={{ fontSize: '11px', color: colors.textSecondary }}>{meal.calories} kcal</span>
                        )}
                      </div>
                    </div>
                    {meal.id === current?.id && (
                      <div style={{ width: 22, height: 22, borderRadius: 11, background: colors.accentPurple, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{
              position: 'sticky',
              bottom: 0,
              background: preferences.darkMode ? 'rgba(17,17,17,0.96)' : 'rgba(255,255,255,0.96)',
          ...
          <div className="w-full max-w-sm rounded-3xl p-6" style={{ background: colors.card }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input type="text" value={newGroceryName} onChange={(e) => setNewGroceryName(e.target.value)}
                ...
                style={{
                  ...
                  border: `1.5px solid ${colors.border}`, fontSize: '15px', outline: 'none',
                }}
                onFocus={(e) => e.target.style.borderColor = colors.accentPurpleLight}
                onBlur={(e) => e.target.style.borderColor = colors.border} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label className="section-label" style={{ display: 'block', marginBottom: 6 }}>Unit (optional)</label>
              <input type="text" value={newGroceryUnit} onChange={(e) => setNewGroceryUnit(e.target.value)}
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowAddGrocery(false)}
                ...
                style={{ border: `1px solid ${colors.border}`, background: colors.card, fontSize: '14px', fontWeight: 600, color: colors.textTertiary }}>
                Cancel
              </button>
              <button onClick={handleAddGrocery} disabled={!newGroceryName.trim()}
                className="flex-1 py-3 rounded-xl border-none cursor-pointer"
                style={{ background: colors.accentPurple, color: '#FFF', fontSize: '14px', fontWeight: 600, opacity: newGroceryName.trim() ? 1 : 0.4 }}>
                Add
              </button>
            </div>
          </div>
...
/* ─── PLANNER MEAL CARD — consistent with Home ─── */
function PlannerMealCard({
  colors, type, meal, isDone, isSkipped: _isSkipped, onNavigate, onRotate, onSwap,
}: {
  colors: typeof lightColors
  type: string; meal: Meal
  isDone?: boolean; isSkipped?: boolean
  onNavigate: () => void; onRotate: () => void; onSwap: () => void
}) {
  ...
  return (
    <div style={{ opacity: 1 }}>
      <div style={{
        ...
        >
          <div style={{ padding: '20px 22px 0' }}>
              ...
              {meal.calories && (
                <div
                  style={{
                    ...
                    padding: '6px 10px',
                    borderRadius: 999,
                    background: colors.pageSurface === '#121212' ? 'rgba(154, 77, 90, 0.18)' : colors.accentSoft,
                    color: colors.textPrimary,
                    fontSize: '11px',
                    fontWeight: 600,
                  }}
            ...
            <h2 style={{
              ...
              color: colors.textPrimary, margin: '0 0 8px 0', lineHeight: 1.15,
              maxWidth: '100%',
            }}>
              {meal.name}
            </h2>
            <p style={{
              fontSize: '13px', color: colors.textSecondary, fontStyle: 'italic',
          ...
          {showImage ? (
            <div style={{ margin: '16px 14px 14px', height: 200, overflow: 'hidden', borderRadius: 16, position: 'relative' }}>
              <img src={meal.image} alt={meal.name} onError={() => setImgError(true)}
        ...
        <div className="flex items-center gap-4" style={{ padding: showImage ? '0 22px 16px' : '0 22px 18px' }}>
            <button
              onClick={onRotate}
              className="flex items-center gap-1 bg-transparent border-none cursor-pointer p-0"
              style={{ fontSize: '12px', fontWeight: 500, color: colors.textSecondary }}
            >
              <RefreshCw size={12} />
              Rotate
          </button>
          <button
            onClick={onSwap}
            className="flex items-center gap-1 bg-transparent border-none cursor-pointer p-0"
            style={{ fontSize: '12px', fontWeight: 600, color: tertiaryActionColor }}
          >
            <Repeat2 size={12} />
            Change
          </button>
