...
export default function Onboarding() {
  ...
  const ListCard = ({
    ...
    <button onClick={onClick}
      ...
      style={{
        background: selected ? '#F8F6F3' : '#F6F6F6',
        border: selected ? '1.5px solid rgba(60, 21, 26, 0.5)' : '1px solid #F4F4F4',
      }}>
      <span style={{ fontSize: '28px', lineHeight: 1, flexShrink: 0 }}>{emoji}</span>
      <div className="flex-1 text-left">
        <span style={{ fontSize: '15px', fontWeight: 600, color: selected ? '#3C151A' : '#111111', display: 'block' }}>
          {label}
        </span>
        {desc && (
          <span style={{ fontSize: '12px', color: '#7A746D', marginTop: 2, display: 'block' }}>{desc}</span>
        )}
      </div>
      {trailing || (selected && (
        <div style={{
          width: 24, height: 24, borderRadius: 12,
          background: '#3C151A', display: 'flex', alignItems: 'center', justifyContent: 'center',
          flexShrink: 0,
        }}>
      ...
      {step === 0 && (
        <div className="flex-1 flex flex-col px-6" style={{ paddingTop: 60, paddingBottom: 40 }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <p style={{ fontSize: '14px', color: '#7A746D', fontWeight: 500, margin: '0 0 10px 0' }}>
              Meal plans, sorted ✨
            </p>
            <h1 style={{ ...serifStyle, fontSize: '34px', fontWeight: 400, color: '#111111', lineHeight: 1.2, margin: '0 0 32px 0' }}>
          ...
          <div style={{
            ...
            padding: '28px 24px',
            boxShadow: '0 4px 24px rgba(31,30,46,0.04)',
            border: '1px solid #ECE8E4',
            marginBottom: 'auto',
          }}>
            <h2 style={{ ...serifStyle, fontSize: '22px', fontWeight: 400, color: '#111111', margin: '0 0 6px 0' }}>
              Let's sort your meals
            </h2>
            <p style={{ fontSize: '13px', color: '#7A746D', lineHeight: 1.5, margin: '0 0 22px 0' }}>
            ...
            <button onClick={handleSignIn}
              ...
              style={{ background: '#111111', border: 'none', marginBottom: 10 }}>
            ...
            <button onClick={handleSignIn}
              className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl cursor-pointer transition-smooth"
              style={{ background: '#FFFFFF', border: '1px solid #ECE8E4', marginBottom: 16 }}>
              ...
              <span style={{ fontSize: '15px', fontWeight: 600, color: '#111111' }}>Continue with Google</span>
            </button>

            <p style={{ fontSize: '11px', color: '#7A746D', textAlign: 'center', lineHeight: 1.5, margin: 0 }}>
      ...
      {step > 0 && (
        <>
            ...
            <div className="flex gap-1.5 mb-6">
              {Array.from({ length: TOTAL_STEPS - 1 }).map((_, i) => (
                <div key={i} className="h-1 flex-1 rounded-full transition-all" style={{
                  background: i < step ? '#3C151A' : '#ECE8E4',
                }} />
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 px-6 pb-32 overflow-y-auto">
            ...
            {step === 1 && (
              <div>
                <h1 style={{ ...serifStyle, fontSize: '28px', fontWeight: 400, color: '#111111', lineHeight: 1.2, margin: '0 0 8px 0' }}>
                  What's your food style?
                </h1>
                <p style={{ fontSize: '14px', color: '#7A746D', lineHeight: 1.5, margin: '0 0 24px 0' }}>
              ...
              <div>
                ...
                <div className="flex gap-3 mb-5">
                  <button onClick={() => setHasCook(true)}
                    ...
                    style={{
                      background: hasCook === true ? '#F8F6F3' : '#F6F6F6',
                      border: hasCook === true ? '1.5px solid rgba(60, 21, 26, 0.5)' : '1px solid #F4F4F4',
                    }}>
                    <span style={{ fontSize: '32px', lineHeight: 1 }}>👨‍🍳</span>
                    <span style={{ fontSize: '14px', fontWeight: 600, color: hasCook === true ? '#3C151A' : '#111111' }}>
                      Yes, lifesaver!
                    </span>
                  </button>
                  <button onClick={() => setHasCook(false)}
                    className="flex-1 flex flex-col items-center gap-2 py-6 rounded-2xl cursor-pointer transition-smooth"
                    style={{
                      background: hasCook === false ? '#F8F6F3' : '#F6F6F6',
                      border: hasCook === false ? '1.5px solid rgba(60, 21, 26, 0.5)' : '1px solid #F4F4F4',
                    }}>
                    <span style={{ fontSize: '32px', lineHeight: 1 }}>🧑‍🍳</span>
                    <span style={{ fontSize: '14px', fontWeight: 600, color: hasCook === false ? '#3C151A' : '#111111' }}>
                      I cook myself
                    </span>
                  </button>
                </div>
                ...
                {hasCook === true && (
                  <div style={{
                    background: '#F6F6F6', borderRadius: 20, padding: '20px 18px',
                    border: '1px solid #ECE8E4',
                  }}>
                    <p style={{ fontSize: '13px', color: '#7A746D', margin: '0 0 16px 0' }}>
                      Great! Add their details so we can send meal plans via WhatsApp.
                    </p>
                    <div className="space-y-4">
                      <div>
                        <label style={{ fontSize: '12px', fontWeight: 600, color: '#7A746D', display: 'block', marginBottom: 6 }}>
                        ...
                        <input type="text" value={cookName} onChange={(e) => setCookName(e.target.value)}
                          ...
                          style={{
                            ...
                            border: `1px solid ${cookName.trim() === '' ? '#ECE8E4' : '#3C151A'}`,
                            background: '#FFFFFF', fontSize: '14px', outline: 'none',
                            fontFamily: 'inherit', color: '#111111',
                          }}
                          onFocus={(e) => e.target.style.borderColor = '#3C151A'} onBlur={(e) => e.target.style.borderColor = cookName.trim() ? '#3C151A' : '#ECE8E4'} />
                        {cookName.trim() === '' && (
                          <p style={{ fontSize: '11px', color: '#7A746D', margin: '4px 0 0 0', lineHeight: 1.4 }}>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex items-center px-3 py-3 rounded-xl" style={{ background: '#F1F2F4', border: '1px solid #ECE8E4' }}>
                            <span style={{ fontSize: '13px', color: '#7A746D', fontWeight: 500 }}>+91</span>
                          </div>
                          <input type="tel" value={cookPhone} onChange={(e) => {
                            ...
                            style={{
                              ...
                              border: `1px solid ${cookPhone.trim().length >= 10 ? '#3C151A' : '#ECE8E4'}`,
                              background: '#FFFFFF', fontSize: '14px', outline: 'none',
                              fontFamily: 'inherit', color: '#111111',
                            }}
                            onFocus={(e) => e.target.style.borderColor = '#3C151A'} onBlur={(e) => e.target.style.borderColor = cookPhone.trim().length >= 10 ? '#3C151A' : '#ECE8E4'} />
                        </div>
                        <p style={{ fontSize: '11px', color: '#7A746D', margin: '6px 0 0 0', lineHeight: 1.4 }}>
                  ...
                  {GROCERY_APPS.map((app) => {
                    ...
                    return (
                      <button key={app.id} onClick={() => toggle(groceryApps, app.id, setGroceryApps)}
                        ...
                        <span className="flex-1 text-left" style={{ fontSize: '15px', fontWeight: 600, color: sel ? '#3C151A' : '#111111' }}>
                          {app.name}
                        </span>
                        {sel ? (
                          <div style={{
          ...
          <div className="fixed bottom-0 left-0 right-0 p-5" style={{
            ...
            <div className="max-w-md mx-auto flex gap-3">
              <button onClick={() => setStep(step - 1)}
                ...
                style={{ background: '#F6F6F6', border: '1px solid #ECE8E4', flexShrink: 0 }}>
              ...
              <button onClick={() => {
                ...
                style={{
                  background: (canProceed() || step === 2 || step === 5) ? '#3C151A' : '#ECE8E4',
                  color: (canProceed() || step === 2 || step === 5) ? '#FFFFFF' : '#7A746D',
                  fontSize: '15px',
                  opacity: (canProceed() || step === 2 || step === 5) ? 1 : 0.85,
                }}>
